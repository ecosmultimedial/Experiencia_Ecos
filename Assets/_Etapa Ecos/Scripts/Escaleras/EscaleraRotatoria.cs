...
public class EscaleraRotatoria : MonoBehaviour
{
    ...
    {
        if (estaRotando || !enPosicionB) return;
        // Para la vuelta usamos el ángulo de vuelta, NO simplemente "volver a rotacionA"
        // así podemos elegir por qué camino gira.
    ...
    private IEnumerator RotarRutina(Quaternion rotacionFinal, bool destinoEsB)
    {
        ...
        // Aseguramos que quede exactamente alineada
        if (destinoEsB)
            transform.rotation = rotacionFinal;
        else
            transform.rotation = rotacionA; // al volver, snap exacto a la rotación original
